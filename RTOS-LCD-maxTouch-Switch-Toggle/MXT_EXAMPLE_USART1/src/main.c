...
/* prototypes                                                           */
/************************************************************************/
void but0_callback(t_but);
void but1_callback(t_but);
void but2_callback(t_but);
void but3_callback(t_but);
void but4_callback(t_but);
void draw_button_new(t_but);
...
int process_touch(t_but botoes[], touchData touch, uint32_t n){
	for (int i=0; i < n; i++){
		if ((touch.x >= botoes[i].x) && (touch.x <= (botoes[i].x+botoes[i].width)) && (touch.y >= botoes[i].y) && (touch.y <= botoes[i].y+botoes[i].height)){
			printf("ENTROU para o botao:");
			printf("x:%d\n", i);
			return i;
		}
	}
