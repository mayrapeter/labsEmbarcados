...
const float32_t firCoeffs32[NUM_TAPS] ={0.07930125683894955,
	0.08147535648783032,
	0.08323976516671625,
	0.08457786363832452,
	0.08547702101550535,
...
void task_lcd(void){
  ...
  while (true) {
    ...
    if (xQueueReceive( xQueuePlot, &(plot), ( TickType_t )  100 / portTICK_PERIOD_MS)) {     
		...
		value = 360*plot.raw/maxi;
		
		if ((x + 5) >= ILI9488_LCD_WIDTH){
			x = 0;	
			draw_screen();
		}
		else{
		...
		ili9488_draw_filled_circle(ILI9488_LCD_WIDTH/2 + raio*cos(value*PI/180), ILI9488_LCD_HEIGHT/2 + raio*sin(value*PI/180), 2 );
    }
  }    
}
